  ...
  )
{
  ...
  if (IsRead) {
    ...
    SdMmcCmdBlk.CommandIndex = SD_WRITE_SINGLE_BLOCK;
    SdMmcCmdBlk.CommandType  = SdMmcCommandTypeAdtc;
    SdMmcCmdBlk.ResponseType = SdMmcResponseTypeR1;
  }
  ...
  if (Device->SectorAddressing) {
    ...
    SdMmcCmdBlk.CommandArgument = (UINT32)MultU64x32 (Lba, Device->BlockMedia.BlockSize);
  }
  ...
  )
{
  ...
  if (IsRead) {
    ...
    SdMmcCmdBlk.CommandIndex = SD_WRITE_MULTIPLE_BLOCK;
    SdMmcCmdBlk.CommandType  = SdMmcCommandTypeAdtc;
    SdMmcCmdBlk.ResponseType = SdMmcResponseTypeR1;
  }
  ...
  )
{
  ...
  while (Remaining > 0) {
    ...
    if (BlockNum == 1) {
      Status = SdRwSingleBlock (Device, Lba, Buffer, BufferSize, IsRead);
    } else {
      Status = SdRwMultiBlocks (Device, Lba, Buffer, BufferSize, IsRead);
    }
    if (EFI_ERROR (Status)) {
      return Status;
    }
  ...
  )
{
  ...
  Status = SdReadWrite (Device, MediaId, Lba, Buffer, BufferSize, TRUE);
  return Status;
}
  ...
  )
{
  ...
  Status = SdReadWrite (Device, MediaId, Lba, Buffer, BufferSize, FALSE);
  return Status;
}

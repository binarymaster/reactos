...
const CHAR * EmmcDeviceName = "emmc";
const CHAR * EmmcPartitionDeviceName = "mmcp";
...
ULONG
EmmcDriverRegisterDriver (
	IN PISD_DRIVER_INTERFACE Driver
);

ULONG
EmmcDriverUnregisterDrivers ();

ISD_DRIVER_INTERFACE EmmcDriverInterface = {
	{
		NULL,
		NULL,
	},
	EmmcDriverRegisterDriver,
	EmmcDriverUnregisterDrivers,
	(ISD_DRIVER_PROBE)EmmcDriverBindingSupported,
	(ISD_DRIVER_START)EmmcDriverBindingStart,
	(ISD_DRIVER_STOP)EmmcDriverBindingStop
};
  ...
  )
{
  ...
  for (Index = 0; Index < EMMC_MAX_PARTITIONS; Index++) {
    ...
    Partition->DevicePath.Name = EmmcPartitionDeviceName;
    Partition->DevicePath.Index = Index;
    Partition->DevicePath.Parent = &Device->DevicePath;

    Partition->BlockMedia.DevicePath = &Partition->DevicePath;
...
/**
  Start upper driver for the specified partition in the EMMC device.
  ...
  )
{
  ...
  if (BlockIoDriver == NULL) return EFI_NOT_FOUND;

  Partition = &Device->Partition[Index];
  if (Partition->Enable) {
    Status = IsdProbeAndStartDevice(BlockIoDriver, &Partition->BlockIo, &Partition->DriverContext);
    if (EFI_ERROR (Status)) return Status;
    Partition->Driver = BlockIoDriver;
  } else {
...
/**
  Stop upper driver for the specified partition in the EMMC device.
  ...
  )
{
  ...
  Partition = &Device->Partition[Index];
  if ((Partition->Enable) && (Partition->Driver != NULL)) {
    Status = Partition->Driver->Stop(&Partition->BlockIo, Partition->DriverContext);
    if (EFI_ERROR(Status)) return Status;
    Partition->Driver = NULL;
  } else {
    Status = EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  //
  for (Index = 0; Index < EMMC_MAX_PARTITIONS; Index++) {
    DEBUG((DEBUG_INFO, "Partition %d Enable %d BlockSize %d LastBlock %I64d\n", Index, Device->Partition[Index].Enable, Device->Partition[Index].BlockMedia.BlockSize, Device->Partition[Index].BlockMedia.LastBlock));
    if ((Index == EmmcPartitionUserData) && (Device->Partition[Index].Enable)) StartDriverOnPartition (Device, Index);
  }
...
**/
EFI_STATUS
EmmcDriverBindingSupported (
  IN EFI_SD_MMC_DEVICE *Device
  )
{
  if (Device->Slot->CardType != EmmcCardType) return EFI_UNSUPPORTED;
...
**/
EFI_STATUS
EmmcDriverBindingStop (
  IN EFI_SD_MMC_DEVICE *Device,
  IN EMMC_DRIVER_PRIVATE_DATA * Private
  )
{
  EFI_STATUS                      Status;
  EMMC_DEVICE                     *EmmcDevice;
  EMMC_PARTITION                  *Partition;
  UINT8                           Index;

  if (!Private) return EFI_DEVICE_ERROR;

  EmmcDevice = &Private->Device;

  for (Index = 0; Index < EMMC_MAX_PARTITIONS; Index++) {
    Partition = &EmmcDevice->Partition[Index];
    if ((Partition->Enable) && (Partition->Driver != NULL)) {
	  Status = StopDriverOnPartition(EmmcDevice, Index);
	  if (EFI_ERROR (Status)) return Status;
	}
  }
  FreePool(Private);

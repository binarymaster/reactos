...
ULONG CompBattDebug = COMPBATT_DEBUG_ALL_SWITCHES;
...
{
    PAGED_CODE();
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING OpenClose\n");
    ...
    /* Return success */
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: Exiting OpenClose\n");
                      ...
                      IN PIRP Irp)
{
    ...
    NTSTATUS Status;
    PAGED_CODE();
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING System Control\n");
    ...
    _In_ PCOMPBATT_BATTERY_DATA BatteryData)
{
    ...
    switch (Mode)
    {
        ...
        case COMPBATT_READ_STATUS:
        {
             ...
             * otherwise the request must be re-issued again. This tipically
             * happens if the wait values are in conflict which it might
             * end up in incosistent battery status results.
             */
        ...
        default:
        {
            ...
            {
                DbgPrint("CompBatt: BAD!!! WE SHOULD NOT BE HERE!\n");
            }
    ...
    /* Setup the necessary datum to read battery status */
...
CompBattRecalculateTag(IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    ...
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING CompBattRecalculateTag\n");
    ...
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: EXITING CompBattRecalculateTag\n");
}
              ...
              IN PIRP Irp)
{
    PCOMPBATT_DEVICE_EXTENSION DeviceExtension = DeviceObject->DeviceExtension;
    NTSTATUS Status;
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING Ioctl\n");
    ...
    /* Return status */
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: EXITING Ioctl\n");
    return Status;
}
                 ...
                 OUT PULONG Tag)
{
    NTSTATUS Status;
    PAGED_CODE();
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING QueryTag\n");
    ...
    /* Return status */
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: EXITING QueryTag\n");
    return Status;
}
...
CompBattDisableStatusNotify(IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    PCOMPBATT_BATTERY_DATA BatteryData;
    PLIST_ENTRY ListHead, NextEntry;
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING DisableStatusNotify\n");
    ...
    while (NextEntry != ListHead)
    {
        ...
        BatteryData->WaitStatus.HighCapacity = COMPBATT_WAIT_MAX_HIGH_CAPACITY;
        NextEntry = NextEntry->Flink;
    }

    /* Done */
    ExReleaseFastMutex(&DeviceExtension->Lock);
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: EXITING DisableStatusNotify\n");
    return STATUS_SUCCESS;
}
    ...
    _Out_ PULONG BatteriesCount)
{
    ...
    while (NextEntry != ListHead)
    {
        ...
        /* Move on to the next linked battery */
        NextEntry = NextEntry->Flink;
    }
    ...
    _In_ PBATTERY_NOTIFY BatteryNotify)
{
    ...
    /* Cache the notification parameters provided for latre usage when polling for battery status */
    ...
    while (NextEntry != ListHead)
    {
        ...
        if (NT_SUCCESS(IoAcquireRemoveLock(&BatteryData->RemoveLock, BatteryData->Irp)))
        {
            ...
            if (BatteryData->Tag != BATTERY_TAG_INVALID)
            {
                ...
                /* Always pick up the greatest rate for the composite battery */
                if (BatteryData->BatteryStatus.Rate != BATTERY_UNKNOWN_RATE)
                {
                    BatteryStatus->Rate = max(BatteryStatus->Rate,
                                              BatteryData->BatteryStatus.Rate);
                }
            }
                              ...
                              IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    ...
    PCOMPBATT_BATTERY_DATA BatteryData;
    PLIST_ENTRY ListHead, NextEntry;
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING GetBatteryInformation\n");
    ...
    /* We are done */
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: EXITING GetBatteryInformation\n");
    return Status;
}
                              ...
                              IN PCOMPBATT_DEVICE_EXTENSION DeviceExtension)
{
    ...
    PLIST_ENTRY ListHead, NextEntry;
    ULONG i;
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: ENTERING GetBatteryGranularity\n");
    ...
    /* All done */
    ExReleaseFastMutex(&DeviceExtension->Lock);
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: EXITING GetBatteryGranularity\n");
    return STATUS_SUCCESS;
}
    ...
    while (NextEntry != ListHead)
    {
        ...
        if (NT_SUCCESS(IoAcquireRemoveLock(&BatteryData->RemoveLock, BatteryData->Irp)))
        {
            ...
            if (BatteryData->Tag != BATTERY_TAG_INVALID)
            {
                ...
                if (NT_SUCCESS(Status))
                {
                    if (Time && Time != BATTERY_UNKNOWN_TIME)
                    {
                        AtRateCapacity = BatteryData->BatteryStatus.Capacity * COMPBATT_ATRATE_CAPACITY_CONSTANT;
                        ComputedAtRate -= AtRateCapacity / Time;
                    }
                }
            }
                         ...
                         OUT PULONG ReturnedLength)
{
    ...
    NTSTATUS Status = STATUS_SUCCESS;
    PAGED_CODE();
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT)  DbgPrint("CompBatt: ENTERING QueryInformation\n");
    ...
    *ReturnedLength = QueryLength;
    ...
    if (CompBattDebug & COMPBATT_DEBUG_FUNC_ENTER_EXIT) DbgPrint("CompBatt: EXITING QueryInformation\n");
    return Status;
}
